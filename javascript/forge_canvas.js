/*
 * Forge Canvas
 * Adapted from Haoming02/sd-webui-forge-classic modules_forge/forge_canvas.
 * Upstream license: AGPL-3.0.
 */
function forgeCanvasT(en, cn) {
    if (window.forgeNeoTranslate) return window.forgeNeoTranslate(en, cn);
    const runtime = document.querySelector(".forge-neo-hidden-runtime");
    const raw = String(runtime ? runtime.getAttribute("data-lang") : "").toLowerCase();
...
class ForgeCanvas {
    ...
    init() {
        ...
        scribbleWidthLabel.textContent = `${forgeCanvasT("Brush Width", "画笔宽度")} (${self.scribbleWidth})`;
        ...
        scribbleAlphaLabel.textContent = `${forgeCanvasT("Brush Opacity", "画笔透明度")} (${self.scribbleAlpha})`;
        ...
        scribbleSoftnessLabel.textContent = `${forgeCanvasT("Brush Softness", "画笔柔和度")} (${self.scribbleSoftness})`;
        ...
        function resetScribble(e, rect) {
            const displayScale = self.canvasDisplayScale(drawingCanvas);
            const indicatorSize = self.scribbleWidth * (self.scribbleWidthConsistent ? 1.0 : displayScale) * 4;
            scribbleIndicator.style.width = `${indicatorSize}px`;
            scribbleIndicator.style.height = `${indicatorSize}px`;
        ...
        scribbleWidth.addEventListener("input", (e) => {
            ...
            scribbleWidthLabel.textContent = `${forgeCanvasT("Brush Width", "画笔宽度")} (${self.scribbleWidth})`;
            const indicatorSize = self.scribbleWidth * (self.scribbleWidthConsistent ? 1.0 : self.canvasDisplayScale(drawingCanvas)) * 4;
            scribbleIndicator.style.width = `${indicatorSize}px`;
            scribbleIndicator.style.height = `${indicatorSize}px`;
        });
        ...
        drawingCanvas.addEventListener("pointerdown", (e) => {
            ...
            const point = self.canvasPointFromEvent(drawingCanvas, e);
            self.temp_draw_points = [[point.x, point.y]];
    ...
    handleDraw(e) {
        ...
        const point = this.canvasPointFromEvent(canvas, e);
        const x = point.x;
        const y = point.y;
        ...
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        ctx.lineWidth = this.scribbleWidth / (this.scribbleWidthConsistent ? this.canvasDisplayScale(canvas) : 1.0) * 4;
    ...
    drawImage() {
        ...
        if (this.img) {
            ...
            const scaledHeight = this.orgHeight * this.imgScale;
            image.src = this.img;
            image.style.setProperty("width", `${scaledWidth}px`, "important");
            image.style.setProperty("height", `${scaledHeight}px`, "important");
            image.style.setProperty("left", `${this.imgX}px`);
            image.style.setProperty("top", `${this.imgY}px`);
            image.style.display = "block";
            drawingCanvas.style.setProperty("width", `${scaledWidth}px`, "important");
            drawingCanvas.style.setProperty("height", `${scaledHeight}px`, "important");
            drawingCanvas.style.setProperty("left", `${this.imgX}px`);
            drawingCanvas.style.setProperty("top", `${this.imgY}px`);
        } else {
            image.src = "";
    ...
    adjustInitialPositionAndScale() {
        ...
        if (!container || !this.orgWidth || !this.orgHeight) {
            this.imgScale = 1.0;
            this.imgX = 0;
            this.imgY = 0;
            return;
