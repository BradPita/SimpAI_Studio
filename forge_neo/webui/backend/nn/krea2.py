...
class RMSNorm(nn.Module):
    def __init__(self, features: int, eps: float = 1e-5, device=None, dtype=None, **kwargs):
        super().__init__()
        self.eps = eps
...
class QKNorm(nn.Module):
    def __init__(self, dim: int, device=None, dtype=None, **kwargs):
        super().__init__()
        self.qnorm = RMSNorm(dim, device=device, dtype=dtype)
        self.knorm = RMSNorm(dim, device=device, dtype=dtype)
...
class SwiGLU(nn.Module):
    def __init__(self, features: int, multiplier: int, bias: bool = False, multiple: int = 128, device=None, dtype=None, **kwargs):
        super().__init__()
        mlpdim = int(2 * features / 3) * multiplier
        mlpdim = multiple * ((mlpdim + multiple - 1) // multiple)
        self.gate = nn.Linear(features, mlpdim, bias=bias, device=device, dtype=dtype)
        self.up = nn.Linear(features, mlpdim, bias=bias, device=device, dtype=dtype)
        self.down = nn.Linear(mlpdim, features, bias=bias, device=device, dtype=dtype)
...
class Attention(nn.Module):
    def __init__(self, dim: int, heads: int, kvheads: Optional[int] = None, bias: bool = False, device=None, dtype=None, **kwargs):
        super().__init__()
        self.heads = heads
        self.kvheads = kvheads if kvheads is not None else heads
        self.headdim = dim // self.heads
        self.wq = nn.Linear(dim, self.headdim * self.heads, bias=bias, device=device, dtype=dtype)
        self.wk = nn.Linear(dim, self.headdim * self.kvheads, bias=bias, device=device, dtype=dtype)
        self.wv = nn.Linear(dim, self.headdim * self.kvheads, bias=bias, device=device, dtype=dtype)
        self.gate = nn.Linear(dim, dim, bias=bias, device=device, dtype=dtype)
        self.qknorm = QKNorm(self.headdim, device=device, dtype=dtype)
        self.wo = nn.Linear(dim, dim, bias=bias, device=device, dtype=dtype)
...
class TextFusionBlock(nn.Module):
    def __init__(self, features, heads, multiplier, bias=False, kvheads=None, device=None, dtype=None, **kwargs):
        super().__init__()
        self.prenorm = RMSNorm(features, device=device, dtype=dtype)
        self.postnorm = RMSNorm(features, device=device, dtype=dtype)
        self.attn = Attention(features, heads, kvheads=kvheads, bias=bias, device=device, dtype=dtype)
        self.mlp = SwiGLU(features, multiplier, bias, device=device, dtype=dtype)
...
class TextFusionTransformer(nn.Module):
    def __init__(self, num_txt_layers, txt_dim, heads, multiplier, bias=False, kvheads=None, device=None, dtype=None, **kwargs):
        super().__init__()
        self.layerwise_blocks = nn.ModuleList(
            [
                TextFusionBlock(txt_dim, heads, multiplier, bias, kvheads, device=device, dtype=dtype)
                for _ in range(2)
            ]
        )
        self.projector = nn.Linear(num_txt_layers, 1, bias=False, device=device, dtype=dtype)
...
class SingleStreamBlock(nn.Module):
    def __init__(self, features, heads, multiplier, bias=False, kvheads=None, device=None, dtype=None, **kwargs):
        super().__init__()
        self.mod = DoubleSharedModulation(features, device=device, dtype=dtype)
        self.prenorm = RMSNorm(features, device=device, dtype=dtype)
        self.postnorm = RMSNorm(features, device=device, dtype=dtype)
        self.attn = Attention(features, heads, kvheads=kvheads, bias=bias, device=device, dtype=dtype)
        self.mlp = SwiGLU(features, multiplier, bias, device=device, dtype=dtype)
...
class LastLayer(nn.Module):
    def __init__(self, features, patch, channels, device=None, dtype=None, **kwargs):
        super().__init__()
        self.norm = RMSNorm(features, device=device, dtype=dtype)
        self.linear = nn.Linear(features, patch * patch * channels, bias=True, device=device, dtype=dtype)
        self.modulation = SimpleModulation(features, device=device, dtype=dtype)
...
class SingleStreamDiT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.first = nn.Linear(channels * patch**2, features, bias=True, device=device, dtype=dtype)
        self.blocks = nn.ModuleList(
            [
                SingleStreamBlock(features, heads, multiplier, bias, kvheads, device=device, dtype=dtype)
                for _ in range(layers)
            ]
        )
        self.tmlp = nn.Sequential(
            nn.Linear(tdim, features, device=device, dtype=dtype),
            nn.GELU(approximate="tanh"),
            nn.Linear(features, features, device=device, dtype=dtype),
        )
        ...
        self.txtmlp = nn.Sequential(
            ...
            nn.Linear(txtdim, features, device=device, dtype=dtype),
            nn.GELU(approximate="tanh"),
            nn.Linear(features, features, device=device, dtype=dtype),
        )
        ...
        self.tproj = nn.Sequential(
            nn.GELU(approximate="tanh"),
            nn.Linear(features, features * 6, device=device, dtype=dtype),
        )
    ...
    def forward(self, x, timesteps, context, attention_mask=None, transformer_options={}, **kwargs):
        ...
        imgids = torch.zeros(h_tokens, w_tokens, 3, device=device, dtype=torch.float32)
        imgids[..., 1] = torch.arange(h_tokens, device=device, dtype=torch.float32)[:, None]
        imgids[..., 2] = torch.arange(w_tokens, device=device, dtype=torch.float32)[None, :]
        imgpos = imgids.reshape(1, h_tokens * w_tokens, 3).repeat(bs, 1, 1)
        ...
        out = rearrange(out, "b (h w) (c ph pw) -> b c (h ph) (w pw)", h=h_tokens, w=w_tokens, ph=patch, pw=patch, c=self.channels)
