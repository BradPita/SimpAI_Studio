...
from .scail2_loop_inputs import (
    cleanup_stale_input_cache_dirs,
    is_loop_sequence,
...
class _Scail2LoopSequenceReader:
    def __init__(self, source, dim, storage_device, name):
        ...
        if self.sequence:
            if self.dim != 0:
                raise ValueError("SCAIL-2 loop input sequence metadata only supports dim=0")
            self.segments = list(source.get("segments", []))
            shape = tuple(source.get("shape", ()))
            if len(shape) == 0 or int(shape[0]) <= 0:
                raise ValueError(f"SCAIL-2 {name} sequence is empty")
            self.total_length = int(shape[0])
            ...
            if self.last_frame is None and self.last_frame_path is None:
                raise ValueError(f"SCAIL-2 {name} sequence is missing last-frame metadata")
            return

        if not isinstance(source, torch.Tensor):
            raise TypeError(f"SCAIL-2 {name} sequence must be a Tensor or disk-backed metadata")
        if source.shape[self.dim] <= 0:
            raise ValueError(f"SCAIL-2 {name} sequence contains no frames")
    ...
    def _slice_tensor(self, start, length):
        ...
        if local_start < 0:
            raise RuntimeError(
                f"SCAIL-2 {self.name} reader was trimmed past requested frame {start} "
    ...
    def _segment_tensor(self, segment):
        ...
        if path is None:
            raise RuntimeError(f"SCAIL-2 {self.name} input segment is neither loaded nor backed by disk")
    ...
    def _slice_segments(self, start, length):
        ...
        for segment in self.segments:
            ...
            if overlap_start > copied_until:
                raise RuntimeError(
                    f"SCAIL-2 {self.name} sequence has a gap before frame {overlap_start}"
                )
            chunk = self._segment_tensor(segment)
            parts.append(chunk.narrow(0, overlap_start - seg_start, overlap_end - overlap_start))
            copied_until = overlap_end
            if copied_until >= end:
                break

        if parts:
            out = torch.cat(parts, dim=0) if len(parts) > 1 else parts[0].clone().contiguous()
        else:
            out = self._get_last_frame().narrow(0, 0, 0).clone()
        if out.shape[0] < length:
            pad_shape = [1] * out.ndim
            pad_shape[0] = length - out.shape[0]
            out = torch.cat([out, self._get_last_frame().repeat(*pad_shape)], dim=0)
        return out

    def _preload_available_segments(self, logger=None, max_segments=None):
        ...
        for segment in self.segments:
            ...
            try:
                ...
                if logger is not None:
                    logger.warning(f"SCAIL-2 loop: failed to load {self.name} input segment {path}: {e}")
                break
            ...
            try:
                ...
                if logger is not None:
                    logger.warning(f"SCAIL-2 loop: loaded {self.name} segment but failed to remove disk cache {path}: {e}")
            loaded += 1
        if loaded and logger is not None:
            logger.info(f"SCAIL-2 loop: loaded {loaded} queued {self.name} input segment(s) into memory")
        return loaded

    def preload_available(self, logger=None, max_segments=None):
        return self._preload_available_segments(logger, max_segments=max_segments)
    ...
    def trim_before(self, global_keep_start, logger=None):
                ...
                if seg_end <= global_keep_start:
                    ...
                    if path is not None:
                        try:
                            ...
                            kept.append(segment)
                            if logger is not None:
                                logger.warning(f"SCAIL-2 loop: failed to remove consumed {self.name} input segment: {e}")
                            continue
                    trimmed += 1
                else:
                    kept.append(segment)
        ...
        if not memory_allows_allocation(tensor_nbytes(suffix)):
            if logger is not None:
                logger.warning(
                    f"SCAIL-2 loop: skipped {self.name} input trim because cloning the remaining suffix "
...
class WanVideoSampler:
    ...
    def process(self, model, image_embeds, shift, steps, cfg, seed, scheduler, riflex_freq_index, text_embeds=None,
        ...
        if scail2_looping:
            ...
            lat_h = int(image_embeds.get("lat_h", noise.shape[2]))
            lat_w = int(image_embeds.get("lat_w", noise.shape[3]))
            scail2_input_cache_path = image_embeds.get("scail2_input_cache_path", None)
            try:
                pose_pixels_reader = _Scail2LoopSequenceReader(
                    image_embeds.pop("scail2_pose_pixels", None),
                    0,
                    offload_device,
                    "pose",
                )
                pose_mask_pixels_reader = _Scail2LoopSequenceReader(
                    image_embeds.pop("scail2_pose_mask_pixels", None),
                    0,
                    offload_device,
                    "pose mask",
                )
                uni3c_render_reader = None
                if uni3c_data is not None and uni3c_data.get("render_latent", None) is not None:
                    uni3c_data = uni3c_data.copy()
                    uni3c_render_reader = _Scail2LoopSequenceReader(
                        uni3c_data.pop("render_latent"),
                        2,
                        offload_device,
                        "Uni3C render latent",
                    )
            except BaseException:
                scail2_input_cache_path = remove_input_cache_dir(scail2_input_cache_path, log)
                raise
            ...
            def _build_chunk_scail_data(base_scail_data, chunk_start, local_freeze_mask, first_chunk):
                ...
                if pose_pixels_reader:
                    pose_window = pose_pixels_reader.slice(chunk_start, chunk_frames).to(device)
                ...
                if pose_mask_pixels_reader:
                    mask_window = pose_mask_pixels_reader.slice(chunk_start, chunk_frames).to(device)
            ...
            def _build_chunk_uni3c_data(chunk_start, first_chunk):
                if uni3c_data is None or not uni3c_render_reader:
                    return None
                ...
                if valid_len > 0:
                    valid = uni3c_render_reader.slice(valid_start, valid_len)
                else:
                    valid = uni3c_render_reader.slice(chunk_start // 4, 1).narrow(2, 0, 0)

                if handoff_latents > 0:
                    if valid.shape[2] > 0:
                        pad_source = valid[:, :, :1]
                    else:
                        pad_source = uni3c_render_reader.slice(chunk_start // 4, 1)
                ...
                chunk_data = {"render_latent": local_render_latent.to(device).clone().contiguous()}
            ...
            try:
                try:
                    cleanup_stale_input_cache_dirs(log, exclude_paths=[scail2_input_cache_path])
            ...
            try:
                for chunk_idx in range(num_chunks):
                    ...
                    if chunk_idx + 1 < num_chunks:
                        next_chunk_start = (chunk_idx + 1) * stride_frames
                        pose_trimmed = pose_pixels_reader.trim_before(next_chunk_start, log)
                        pose_pixels_reader.preload_available(log, max_segments=pose_trimmed)
                        mask_trimmed = pose_mask_pixels_reader.trim_before(next_chunk_start, log)
                        pose_mask_pixels_reader.preload_available(log, max_segments=mask_trimmed)
                        if uni3c_render_reader:
                            uni3c_render_reader.trim_before(next_chunk_start // 4, log)
