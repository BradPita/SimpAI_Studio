...
class WanAnimatePlusSCAIL2Embeds:
    ...
    @staticmethod
    def _estimate_resized_bhwc_bytes(images, width, height):
        if images is None:
            return 0
        return int(images.shape[0]) * int(height) * int(width) * 3 * dtype_nbytes(images.dtype)

    @staticmethod
    def _build_resized_bhwc_sequence(images, width, height, mode, crop, cache_state, name, segment_frames, logger=None):
        images = images[:, :, :, :3]
        if images.shape[0] <= 0:
            raise ValueError("SCAIL-2 loop input sequence contains no frames")
        safe_chunk_size = WanAnimatePlusSCAIL2Embeds._safe_frame_chunk_size(
            images.shape[0],
            images.shape[1],
            images.shape[2],
            width,
        ...
        chunk_size = max(1, min(int(segment_frames), safe_chunk_size))
        segments = []
        last_frame = None
        for start in range(0, images.shape[0], chunk_size):
            chunk = images[start:start + chunk_size]
            ...
            if keep_in_memory:
                stored = resized.detach().to(offload_device).contiguous()
                segments.append({
                    "start": int(start),
    ...
    def process(self, vae, width, height, num_frames, frame_window_size, force_offload, pose_strength, ref_strength,
        ...
        scail2_pose_source = None
        scail2_pose_mask_source = None
        scail2_loop_input_cache_path = None
        if pose_images is not None:
            pose_images = self._fit_sequence(pose_images[:, :, :, :3], num_frames)
            if scail2_looping:
                scail2_pose_source = pose_images
            else:
        ...
        if pose_image_mask is not None:
            if scail2_looping:
                scail2_pose_mask_source = pose_image_mask
            else:
        ...
        if scail2_looping and (scail2_pose_source is not None or scail2_pose_mask_source is not None):
            loop_input_bytes = 0
            if scail2_pose_source is not None:
                loop_input_bytes += self._estimate_resized_bhwc_bytes(scail2_pose_source, W // 2, H // 2)
            if scail2_pose_mask_source is not None:
                loop_input_bytes += self._estimate_resized_bhwc_bytes(scail2_pose_mask_source, W // 2, H // 2)

            use_mixed_input_cache = not memory_allows_allocation(loop_input_bytes)
            if use_mixed_input_cache:
                cache_state = {"path": None, "cleaned": False}
                segment_frames = max(1, frame_window_size - 5)
                try:
                    if scail2_pose_source is not None:
                        scail2_pose_pixels = self._build_resized_bhwc_sequence(
                            scail2_pose_source,
                            W // 2,
                            H // 2,
                            "lanczos",
                    ...
                    if scail2_pose_mask_source is not None:
                        scail2_pose_mask_pixels = self._build_resized_bhwc_sequence(
                            scail2_pose_mask_source,
                            W // 2,
                            H // 2,
                            "area",
                            "disabled",
                    ...
                    scail2_loop_input_cache_path = cache_state.get("path")
                    if scail2_loop_input_cache_path is not None:
                        log.info(f"SCAIL-2 loop: using mixed resized pose/mask inputs with temporary cache {scail2_loop_input_cache_path}")
                    else:
                        log.info("SCAIL-2 loop: using mixed resized pose/mask inputs fully in memory")
                except BaseException:
                    scail2_loop_input_cache_path = remove_input_cache_dir(cache_state.get("path"), log)
                    raise
            else:
                if scail2_pose_source is not None:
                    scail2_pose_pixels = self._resize_bhwc(scail2_pose_source, W // 2, H // 2).to(offload_device)
                if scail2_pose_mask_source is not None:
                    scail2_pose_mask_pixels = self._resize_bhwc(scail2_pose_mask_source, W // 2, H // 2, mode="area").to(offload_device)
        ...
        if scail2_loop_input_cache_path is not None:
            image_embeds["scail2_input_cache_path"] = scail2_loop_input_cache_path
        return (image_embeds,)
