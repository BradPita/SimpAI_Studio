...
class PromptTranslate(LLMNodeBase, io.ComfyNode):
    ...
    @classmethod
    def _translate_with_third_party(cls, text, from_lang, to_lang, service_name, from_lang_name, to_lang_name, unique_id):
        """使用 Third-party APIs 翻译服务"""
        request_id = generate_request_id("trans", "third_party", unique_id)

        log_prepare(TASK_TRANSLATE, request_id, SOURCE_NODE, "Third-party APIs", None, None, {"方向": f"{from_lang_name}→{to_lang_name}", "长度": len(text)})
    ...
    def execute(cls, source_text, target_language, translate_service, ollama_auto_unload, seed=None):
        ...
        try:
            ...
            if service_id == 'baidu':
                ...
                request_id, result = cls._translate_with_third_party(
                    source_text, detected_lang, to_lang,
                    translate_service, from_lang_name, to_lang_name, unique_id
                )
            else:
