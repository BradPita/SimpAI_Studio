...
def _normalize_cache_ram_value(ram):
    if isinstance(ram, (list, tuple)):
        return ram[0] if len(ram) > 0 else None
    return ram


CACHE_RAM = None
if args.cache_ram is not None:
    CACHE_RAM = _normalize_cache_ram_value(args.cache_ram)
...
def set_cache_ram(ram):
    global CACHE_RAM
    ram = _normalize_cache_ram_value(ram)
    if CACHE_RAM != ram:
        CACHE_RAM = ram
        try:
            import gc
