...
class SaveImage:
    ...
    def save_images(self, images, filename_prefix="ComfyUI", prompt=None, extra_pnginfo=None):
        ...
        if self.output_dir != folder_paths.get_temp_directory() and self.output_dir != folder_paths.get_output_directory():
            self.output_dir = folder_paths.get_output_directory()
...
class LoadImage:
    ...
    def load_image(self, image):
        ...
        try:
            ...
            if "%" in image_path:
                with open(image_path, "rb") as f:
                    video_source = std_io.BytesIO(f.read())

            components = InputImpl.VideoFromFile(video_source).get_components()
            ...
            logging.debug("Skipping video probe for image file '%s'", image_path, exc_info=True)
