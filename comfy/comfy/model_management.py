...
def print_vram_info_by_nvml(pos=None):
    ...
    if is_nvidia():
        ...
        if memory_info is None:
            logging.debug(f"GPU memory{position}: nvml_unavailable")
            return
        if pid_used_vram:
...
def print_memory_info(pos=None):
    ...
    if is_nvidia():
        ...
        logging.info(f'GPU memory{position}: {pid_and_all_used if pid_and_all_used else ""}max_reserved={max_reserved}, max_allocated={max_allocated}, reserved={reserved}, free={free_cuda}, free_torch={free_torch}, free_total={free_total}, gpu_total={gpu_total}, torch_total={torch_total}')
