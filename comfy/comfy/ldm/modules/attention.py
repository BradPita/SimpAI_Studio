...
_XFORMERS_SKIP_REASONS_LOGGED = set()

def _log_xformers_skip_once(reason):
    if reason in _XFORMERS_SKIP_REASONS_LOGGED:
        return
    _XFORMERS_SKIP_REASONS_LOGGED.add(reason)
    logging.debug("Skipping xformers attention (%s), using pytorch attention.", reason)

def _xformers_pytorch_fallback_reason(q):
    ...
    try:
        ...
        return None
    if capability[0] > 9:
        return f"unsupported-cuda-capability-{capability[0]}.{capability[1]}"
    return None
...
def attention_xformers(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False, **kwargs):
    ...
    if not disabled_xformers:
        xformers_skip_reason = _xformers_pytorch_fallback_reason(q)
        if xformers_skip_reason is not None:
            _log_xformers_skip_once(xformers_skip_reason)
            disabled_xformers = True

    if disabled_xformers:
        return attention_pytorch(q, k, v, heads, mask, attn_precision=attn_precision, skip_reshape=skip_reshape, skip_output_reshape=skip_output_reshape, **kwargs)
