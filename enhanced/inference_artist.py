...
try:
    ...
    logger.warning(
        "[Artist Inference] Triton unavailable; falling back to native PyTorch SKA. error=%s",
        triton_error,
    )


def _native_ska(x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
    ks_sq = int(w.shape[2])
    ks = int(math.sqrt(ks_sq))
    if ks * ks != ks_sq:
        raise ValueError(f"Invalid SKA kernel shape: expected a square kernel, got {ks_sq}.")

    pad = (ks - 1) // 2
    batch, channels, height, width = x.shape
    weight_channels = int(w.shape[1])
    if weight_channels <= 0:
        raise ValueError("Invalid SKA weight tensor: channel dimension must be positive.")

    patches = F.unfold(x, kernel_size=ks, padding=pad)
    patches = patches.view(batch, channels, ks_sq, height, width)

    if weight_channels != channels:
        repeats = (channels + weight_channels - 1) // weight_channels
        w = w.repeat(1, repeats, 1, 1, 1)[:, :channels]

    return (patches * w.to(dtype=patches.dtype)).sum(dim=2)
