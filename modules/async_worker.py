...
def _standard_stream_backup(name):
    stream = getattr(sys, f"__{name}__", None)
    if not _standard_stream_is_closed(stream):
        return stream
    return open(os.devnull, "w", encoding="utf-8", buffering=1)


def _restore_standard_streams_if_closed():
    if _standard_stream_is_closed(sys.stdout):
...
class AsyncTask:
    def __init__(self, args, task_id=None):
        ...
        args.pop()  # legacy api_params positional slot
        args.pop()  # legacy api_params positional slot
        args.pop()  # legacy api_params positional slot
        args.pop()  # legacy api_params positional slot
        ...
        if self.inpaint_engine is None or self.inpaint_engine == '':
            self.inpaint_engine = 'None'
        ...
        if self.simpleai_regen_manifest:
            logger.info(
                "Regen manifest received by AsyncTask: preset=%s theme=%s",
                self.simpleai_regen_manifest.get("preset_name"),
                (self.simpleai_regen_manifest.get("ui_values") or {}).get("scene_theme"),
            )
        self.keep_vlm_model_loaded = bool(self.params_backend.pop('keep_vlm_model_loaded', False))
        logger.info(
            '[VLM KeepLoaded] AsyncTask received task_id=%s task_method=%s task_class=%s keep_vlm_model_loaded=%s',
            self.task_id,
            getattr(self, 'task_method', None),
            getattr(self, 'task_class', None),
            self.keep_vlm_model_loaded,
        )
        ...
        if self.clip_model_name in (None, '', default_clip, default_vae, 'auto'):
            self.params_backend.pop('clip_model', None)
            self.clip_model_name = default_clip
        else:
            self.clip_model_name = str(self.clip_model_name).replace("\\", os.sep).replace("/", os.sep).lstrip(os.sep)
            self.params_backend['clip_model'] = self.clip_model_name
        self.upscale_model_name = self.params_backend.get('upscale_model', 'default')
        if self.upscale_model_name in (None, '', 'auto', 'default'):
            self.params_backend.pop('upscale_model', None)
            self.upscale_model_name = 'default'
        else:
            self.upscale_model_name = str(self.upscale_model_name).replace("\\", os.sep).replace("/", os.sep).lstrip(os.sep)
        ...
        if self.task_class != 'Fooocus':
            ...
            if is_custom_vae:
                ...
                self.params_backend.pop("vae_model", None)
            self.params_backend.update({"is_custom_vae": is_custom_vae})
...
def worker():
    ...
    def yield_result(async_task, imgs, progressbar_index, black_out_nsfw, censor=True, do_not_show_finished_images=False):
        ...
        if do_not_show_finished_images:
            logger.info(
                "[UI-TRACE] yield_result.hidden | total_results=%s, incoming=%s, disable_intermediate_results=%s",
                len(async_task.results),
                len(imgs),
                getattr(async_task, "disable_intermediate_results", None),
            )
            return

        logger.info(
            "[UI-TRACE] yield_result.emit | total_results=%s, incoming=%s, disable_intermediate_results=%s",
            len(async_task.results),
            len(imgs),
            getattr(async_task, "disable_intermediate_results", None),
        )
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras, goals, persist_image=True):
        for index, x in enumerate(imgs):
            ...
            if getattr(async_task, 'simpleai_regen_manifest', None):
                if async_task.save_metadata_to_images:
                    logger.info(
                        ...
                        async_task.simpleai_regen_manifest.get("preset_name"),
                        (async_task.simpleai_regen_manifest.get("ui_values") or {}).get("scene_theme"),
                    )
                else:
                    logger.info(
                        "Regen manifest prepared but embedded metadata saving is disabled: preset=%s theme=%s",
                        async_task.simpleai_regen_manifest.get("preset_name"),
                        (async_task.simpleai_regen_manifest.get("ui_values") or {}).get("scene_theme"),
                    )
                d.append((regen_manifest.LABEL, regen_manifest.KEY, regen_manifest.dumps(async_task.simpleai_regen_manifest)))
            ...
            d.append(('Version', 'version', f'{version.branch}_{version.get_simpai_ver()}'))
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        for i in range(image_number if arrays_mult==0 else arrays_mult):
            ...
            if not async_task.task_method.lower().endswith('_cn') and async_task.task_class not in ['HyDiT', 'Wan', 'Qwen', 'Z-image']:
                ...
                task_extra_negative_prompts = [vlm.translate(pmt, async_task.translation_methods) for pmt in extra_negative_prompts]
    ...
    def process_enhance(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path, controlnet_pose_path, 
        ...
        inpaint_engine = 'None' if inpaint_engine is None or inpaint_engine == '' else str(inpaint_engine)
    ...
    def handler(async_task: AsyncTask):
            ...
            if "scene_" in async_task.task_method:
                ...
                for scene_index in range(1, 5):
                    ...
                    if scene_img is None:
                        continue
                    if isinstance(scene_img, np.ndarray):
                        scene_img = HWC3(scene_img)
                    input_images.set_image(f"i2i_ip_image{scene_index}", scene_img)
                    if scene_index == 1 and "happy_cn" in async_task.task_method and preprocessors.openpose_have(scene_img, ['face']):
            ...
            if "_aio" in async_task.task_method:
                ...
                if async_task.task_class in ('Flux', 'Qwen', 'Wan', 'Z-image', 'Comfy'):
                    ...
                    if refiner_model_name and refiner_model_name != 'None':
                        is_refiner_safetensors_model = _model_type_from_name(refiner_model_name) == 1
                    ...
                    clip_model_name = async_task.params_backend.get('clip_model')
                    if _is_custom_model_choice(clip_model_name):
                        async_task.params_backend['i2i_clip_type'] = _model_type_from_name(clip_model_name)
                    else:
                        async_task.params_backend.pop('i2i_clip_type', None)
                if async_task.task_class == 'Comfy':
                    if 'i2i_uov_tiled_steps' not in async_task.params_backend and async_task.task_method == "sd15_aio":
                        ...
                        async_task.params_backend['display_steps'] = async_task.steps
                elif async_task.task_class in ['Wan', 'Qwen', 'Z-image']:
        ...
        for index, img in enumerate(images_to_enhance):
            ...
            for enhance_mask_dino_prompt_text, enhance_prompt, enhance_negative_prompt, enhance_mask_model, enhance_mask_cloth_category, enhance_mask_sam_model, enhance_mask_text_threshold, enhance_mask_box_threshold, enhance_mask_sam_max_detections, enhance_inpaint_disable_initial_latent, enhance_inpaint_engine, enhance_inpaint_strength, enhance_inpaint_respective_field, enhance_inpaint_erode_or_dilate, enhance_mask_invert in async_task.enhance_ctrls:
                enhance_inpaint_engine = 'None' if enhance_inpaint_engine is None or enhance_inpaint_engine == '' else str(enhance_inpaint_engine)
                current_task_id += 1
                current_progress = int(base_progress + (100 - preparation_steps) / float(all_steps) * (done_steps_upscaling + done_steps_inpainting))
                ...
                if async_task.task_class not in ['HyDiT', 'Wan', 'Qwen', 'Z-image']:
                    enhance_mask_dino_prompt_text = vlm.translate(enhance_mask_dino_prompt_text, async_task.translation_methods)
                    ...
                    if not keep_vlm_model_loaded:
                        ...
                        logger.info(
                            '[VLM KeepLoaded] skip free_model after enhance translation task_id=%s task_method=%s task_class=%s keep_vlm_model_loaded=%s source=async_worker.enhance_translate',
                            getattr(async_task, 'task_id', None),
                            getattr(async_task, 'task_method', None),
                            getattr(async_task, 'task_class', None),
                            keep_vlm_model_loaded,
